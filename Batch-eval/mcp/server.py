...
- 認証: カスタムヘッダー API キー（x-contoso-key）。環境変数 CONTOSO_MCP_KEY が
        設定されている場合のみ検証する。ヘルスチェック（/）は認証不要。
...
API_KEY = os.environ.get("CONTOSO_MCP_KEY")  # 設定時のみ検証
API_KEY_HEADER = "x-contoso-key"
...
class ApiKeyMiddleware(BaseHTTPMiddleware):
    """CONTOSO_MCP_KEY 設定時、/mcp 配下のリクエストにヘッダーキー検証を課す。"""

    async def dispatch(self, request: Request, call_next):
        path = request.url.path
        if API_KEY and path.rstrip("/") == "/mcp":
...
def build_app():
    """Streamable-HTTP の Starlette アプリにヘルスルートと認証を付与して返す。"""

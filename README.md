...
```mermaid
flowchart LR
    subgraph RG["rg-foundryobs-observability (East US 2)"]
...
> - **別名で作りたい / soft-delete 衝突を完全に避けたい**場合は `namePrefix` を変更してください（例: `foundryobs2`）。サフィックスごと別の値になります。
...
4. リポジトリ ルートへの `.env` 生成。あわせて同じターミナル セッションへ環境変数としてエクスポートし、以降の手順でそのまま利用できます。
...
生成して ルートの `.env` に `CONTOSO_MCP_URL` / `CONTOSO_MCP_KEY` として追記します。
...
```powershell
# アプリ名から RG を自動特定（名前を変えた場合は -AppName 相当を修正）
...
./scripts/setup-env.ps1        # ルートの .env から接続情報を引き継いで .env を生成
...
出力された `AGENT_ID`（例 `contoso-support-agent:2`）を このフォルダの `.env` の `AGENT_ID` に追記します。
...
./scripts/setup-env.ps1        # ルートの .env から接続情報（MCP 含む）を引き継いで .env を生成
pip install -r requirements.txt
...
| `namePrefix` | `foundryobs` | リソース名プレフィックス |
| `resourceGroupName` | `rg-foundryobs-observability` | Resource Group 名 |
| `judgeModelName` | `gpt-5.4` | ジャッジ用モデル |
| `judgeModelVersion` | `2026-03-05` | モデルバージョン |
...
- 実際の設定値は リポジトリ ルートの `.env`（**Git 管理外**）に格納します。雛形は
  [`Batch-eval/.env.example`](./Batch-eval/.env.example) を参照。
- 環境構築（`ms-foundry-observability/scripts/deploy.ps1` / `deploy.sh`）は
  リポジトリ ルートに `.env` を生成します。評価コードはこのルートの `.env` を直接読み込みます
  （`PROJECT_ENDPOINT` / `APPLICATIONINSIGHTS_*` など）。コピーは不要です。
